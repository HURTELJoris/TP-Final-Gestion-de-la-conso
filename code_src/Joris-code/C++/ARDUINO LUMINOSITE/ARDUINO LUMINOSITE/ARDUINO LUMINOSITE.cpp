...
ARDUINO_LUMINOSITE::ARDUINO_LUMINOSITE(const std::string& serverAddress, int serverPort, const std::string& serialPort)
...
std::string ARDUINO_LUMINOSITE::createDateTime()
{
...
void ARDUINO_LUMINOSITE::connectSerial(const std::string& port, unsigned int baud_rate) {
    try {
...
std::string ARDUINO_LUMINOSITE::readFromSerial() {
    std::cout << "READ" << std::endl;
    // Créer un objet buffer pour stocker la donnée lue
    boost::asio::streambuf buffer;

    // Lire la donnée à partir du port série en utilisant la fonction read_until
    // qui s'arrête dès qu'elle a lu le délimiteur spécifié ('\n' dans ce cas)
    boost::asio::read_until(serialPort_, buffer, '\n');

    // Extraire la donnée lue du buffer et la convertir en chaîne de caractères
...
void ARDUINO_LUMINOSITE::connectAndSend() {
    try {
        boost::asio::io_context io_context;
...
bool ARDUINO_LUMINOSITE::sendData(tcp::socket& socket) {
    ...
    float luminosity = std::stof(readFromSerial());
...
int main() {
    ...
    ARDUINO_LUMINOSITE sender("192.168.64.88", 1234, "COM6");
