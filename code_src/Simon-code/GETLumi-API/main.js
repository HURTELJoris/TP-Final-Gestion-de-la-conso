document.getElementById('insert-form').addEventListener('submit', async (event) => {
  ...
  try {
      const response = await fetch('http://localhost:8010/api/insertLumi', {
          method: 'POST',
          headers: {
...
async function deleteRow(id) {
  try {
      const response = await fetch(`http://localhost:8010/api/deleteLumi/${id}`, {
          method: 'DELETE',
          headers: {
              'x-api-key': 'root' // Inclusion du mot clé dans l'en-tête
          }
...
async function updateLuminosite(id, luminosite) {
  try {
      const response = await fetch(`http://localhost:8010/api/updateLumi/${id}`, {
          method: 'PUT',
          headers: {
              'Content-Type': 'application/json',
              'x-api-key': 'root' // Inclusion du mot clé dans l'en-tête
...
(async function () {
  try {
      const response = await fetch('http://localhost:8010/api/selectLumi', {
          headers: {
              'x-api-key': 'root' // Inclusion du mot clé dans l'en-tête
          }
      });
